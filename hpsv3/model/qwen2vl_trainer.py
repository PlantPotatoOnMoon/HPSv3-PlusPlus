...
class Qwen2VLRewardModelBT(Qwen2VLForConditionalGeneration):
    ...
    def forward(
        ...
        # Use parent class forward for vision+language processing (compatible with transformers 4.45+4.57)
        outputs = super(Qwen2VLRewardModelBT, self).forward(
            input_ids=input_ids,
        ...
        hidden_states = outputs.hidden_states[-1]  # [B, L, D] from parent forward
...
def _convert_A_B_to_chosen_rejected(
    ...
    """
    # Keep legacy convention for this codebase:
    # image A (the first sample) is always treated as chosen.
    rewards_chosen = rewards_A
    rewards_rejected = rewards_B

    if tied_threshold is None:
        nontied_mask = torch.ones((rewards_A.shape[0], 1), dtype=torch.float32, device=rewards_A.device)
    else:
        nontied_mask = (
            torch.abs(
...
class VLMRewardTrainer(RewardTrainer):
    ...
    def visualize_samples(self, num_print_samples: int):
        """Skip trl 0.16's visualize_samples, which assumes the HF Dataset format and is
        incompatible with the custom PairwiseOriginalDataset + dict eval_dataset."""
        pass
    ...
    def compute_loss(self, model, inputs, return_outputs=False, **kwargs):
        ...
        elif self.loss_type == "max_std_unsup":
            # Unsupervised: maximize reward spread across random pairs within a prompt.
            # Images A and B are randomly drawn from the same prompt (no labels).
            # Loss = -E[(r_A - r_B)^2] -> gradient pushes RM to be more discriminative.
            r_A = rewards_A[:, 0]  # mean reward, shape [B]
            r_B = rewards_B[:, 0]
            loss = -torch.pow(r_A - r_B, 2).mean()
        elif self.loss_type == "min_std_unsup":
            # Unsupervised: minimize reward spread across random pairs within a prompt.
            # Loss = E[(r_A - r_B)^2] -> gradient pushes RM to be more consistent.
            r_A = rewards_A[:, 0]
            r_B = rewards_B[:, 0]
            loss = torch.pow(r_A - r_B, 2).mean()
        else:
    ...
    def _save_checkpoint(self, model, trial, metrics=None):
        ...
        if isinstance(self.model, PeftModel):
            ...
            super(RewardTrainer, self)._save_checkpoint(model, trial)
