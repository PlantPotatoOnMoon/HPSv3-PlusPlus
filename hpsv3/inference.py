...
from .trainer.common import create_model_and_processor
...
class HPSv3RewardInferencer():
    def __init__(self, config_path=None, checkpoint_path=None, device='cuda', differentiable=False):
        ...
        # Select the model class based on the config's model_type: film_hybrid (HPSv3++ Stage 2)
        # requires CapabilityEncoder/FiLM/cond_encoder; everything else (including Stage 1 / original
        # HPSv3) uses the base RM. model_type/cond_dim are not in the ModelConfig dataclass, so read
        # them directly from the yaml.
        import yaml as _yaml
        with open(config_path, "r") as _f:
            _raw_cfg = _yaml.safe_load(_f)
        model_type = _raw_cfg.get("model_type", None)
        if model_type == "film_hybrid":
            from .trainer.adaptive import create_film_hybrid_model_and_processor
            cond_dim = _raw_cfg.get("cond_dim", 256)
    ...
    def reward(self, prompts, image_paths, iter_step=0.0):
        """Score (prompt, image) pairs with the HPSv3++ reward model.

        Conditioning (HPSv3++ / film_hybrid):
          - Model capability is inferred implicitly from the image by the
            CapabilityEncoder; no user input is needed.
          - The RL-iteration condition is passed explicitly via `iter_step`,
            a normalized scalar in [0, 1] following the training convention
            iter_norm = min(1, raw_step / 1000). It defaults to 0.0, i.e. the
            pre-RL (initial-model) setting, which is the recommended value for
            standard preference scoring / reward-model use. When scoring images
            produced at a known RL step during T2I RL fine-tuning, pass the
            corresponding normalized step.
          - `iter_step` may be a scalar (shared by the whole batch) or a list /
            1-D tensor of length B (one value per sample).

        Returns the reward logits; index [i][0] is the mean score (mu) used as
        the final scalar reward for sample i.
        """
        batch = self.prepare_batch(image_paths, prompts)
        bsz = batch["input_ids"].shape[0]
        if not torch.is_tensor(iter_step):
            iter_step = torch.full((bsz,), float(iter_step), dtype=torch.float32)
        iter_values = iter_step.to(self.device).float().reshape(-1)
        if iter_values.numel() == 1 and bsz > 1:
            iter_values = iter_values.expand(bsz)
        # Only the conditioned (film_hybrid) model consumes iter_values; the
        # base reward model ignores the extra kwarg.
        try:
            rewards = self.model(return_dict=True, iter_values=iter_values, **batch)["logits"]
        except TypeError:
